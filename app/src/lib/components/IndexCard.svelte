<script lang="ts">
	...
	import { SOURCES, isMessage, type Contact, type Message, type NoteEntry, type SourceKey } from '$lib/data';
...
<style>
	...
	.touch-icon[data-type='call']     { background: var(--src-signal-bg);   color: var(--src-signal); }
	.touch-icon[data-type='meeting']  { background: var(--accent-wash);     color: var(--accent-ink); }
	.touch-icon[data-type='reminder'] { background: var(--amber-tint);      color: var(--amber-deep); }
	.touch-icon[data-type='note']     { background: var(--src-notes-bg);    color: var(--src-notes); }
	.touch-body {
		flex: 1;
		min-width: 0;
	}
