<script lang="ts">
	...
	let skillsByCategory = $derived.by(() => {
		const groups = new Map<string, SifaSkill[]>();
					...
					{#each sifa.projects as proj, i (i)}
						<article class="card">
							<header class="card-head">
								{#if proj.url}
									<a class="card-title link" href={proj.url} target="_blank" rel="noreferrer">
										<span>{proj.name}</span>
				...
				<ul class="pub-list">
					{#each sifa.publications as pub, i (i)}
						<li class="pub">
							<div class="pub-head">
								{#if pub.url}
									<a class="pub-title link" href={pub.url} target="_blank" rel="noreferrer">
										<span>{pub.title}</span>
...
<style>
	...
	.hero-handle:hover { color: var(--accent-ink); text-decoration: underline; text-underline-offset: 3px; }
	.hero-tag {
		margin-top: 8px;
	...
	.section-title :global(svg) { color: var(--text-muted); }
	.src-mark {
		display: inline-block;
		width: 8px;
		height: 8px;
	...
	.prose.dim { color: var(--text-muted); }
	...
	.t-title { font-weight: 600; letter-spacing: -0.005em; }
	.t-sep { color: var(--text-subtle); }
	.t-org { color: var(--text); }
	.t-meta {
		display: flex;
		flex-wrap: wrap;
		align-items: center;
	...
	.t-when { font-variant-numeric: tabular-nums; }
	.t-tag {
		padding: 2px 8px;
		border-radius: 99px;
	...
	.t-desc.dim { color: var(--text-muted); margin-top: 4px; }
	...
	.card-title.link:hover { color: var(--accent-ink); text-decoration: underline; text-underline-offset: 3px; }
	.card-when {
		font-size: 10.5px;
		color: var(--text-subtle);
		font-variant-numeric: tabular-nums;
	...
	.pub-title.link:hover { color: var(--accent-ink); text-decoration: underline; text-underline-offset: 3px; }
	.pub-meta {
		display: flex;
		flex-wrap: wrap;
		gap: 6px;
	...
	.ext:hover { background: var(--bg-dim); text-decoration: none; }
	.ext-platform {
		font-size: 10px;
		font-weight: 700;
